...
dlc_extension = 'DLC_resnet50_object_space_2021sep9shuffle1_50000.csv'

input_path = os.environ['DATA_DIR_LOCAL'] + ''+f'{mouse}'+'/data/tracking/week'+ f'{session}'+'/'
output_path = os.environ['PROJECT_DIR_LOCAL']+ 'data/compiled_positions/'+f'{mouse}'+'/week'+ f'{session}'+'/'
...
for trial in range(len(session_selection)):
    ...
    print(session_selection.iloc[trial]['raw_file'])
    if type(session_selection.iloc[trial]['raw_file'])==str:
        ...
        x_nose = np.round(tracking_data_array[2:,1].astype(float),2)
        y_nose = np.round(tracking_data_array[2:,2].astype(float),2)
        likelihood_nose = np.round(tracking_data_array[2:,3].astype(float),2)

        x_ear1 = np.round(tracking_data_array[2:,7].astype(float),2)
        y_ear1 = np.round(tracking_data_array[2:,8].astype(float),2)
        likelihood_ear1 = np.round(tracking_data_array[2:,6].astype(float),2)

        x_ear2 = np.round(tracking_data_array[2:,10].astype(float),2)
        y_ear2 = np.round(tracking_data_array[2:,11].astype(float),2)
        likelihood_ear2 = np.round(tracking_data_array[2:,9].astype(float),2)

        x_head = np.round(tracking_data_array[2:,4].astype(float),2)
        y_head = np.round(tracking_data_array[2:,5].astype(float),2)
        likelihood_head = np.round(tracking_data_array[2:,12].astype(float),2)

        x_body = np.round(tracking_data_array[2:, 13].astype(float), 2)
        y_body = np.round(tracking_data_array[2:, 14].astype(float), 2)
        likelihood_body = np.round(tracking_data_array[2:, 15].astype(float), 2)
